    ...
    switch ($operation) {
        ...
        "add" {
			...
			ForEach ($Group in $SourceGroups) {
				Add-LocalGroupMember $Group $accountname
			}
			
			$registry_acl = Get-Acl HKLM:\SYSTEM\CurrentControlSet\Services\EventLog\Security
			$registry_rule = New-Object System.Security.AccessControl.RegistryAccessRule ($accountname,"ReadKey","Allow")
        ...
        "delete" {
			...
			$SourceGroups = "Event Log Readers","Distributed COM Users" 
			ForEach ($Group in $SourceGroups) {
				Remove-LocalGroupMember $Group $accountname
			}
